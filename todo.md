...
**Последняя синхронизация:** 2026-03-18 (актуально)

| Ветка  | Статус | Коммиты впереди | Последний коммит              |
| ------ | ------ | --------------- | ----------------------------- |
| dev    | ✅     | 0               | docs: обновление документации |
...
- `src/app/proxy.ts` — неиспользуемый дубликат middleware
- `middleware.ts` в корне — требует рефакторинга
...
- 🔴 **`src/app/proxy.ts`** — неиспользуемый файл-дубликат (удалить)
- 🔴 **`middleware.ts`** — дубликат логики (переместить в `src/middleware.ts` или удалить)
