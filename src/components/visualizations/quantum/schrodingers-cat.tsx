...
export function SchrodingersCatVisualization({ isDark }: SchrodingersCatVisualizationProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Background
      ctx.fillStyle = isDark ? "#0a0a15" : "#1a1a2e"
      ...
      // Draw box
      ctx.strokeStyle = isDark ? "rgba(100, 100, 150, 0.8)" : "rgba(150, 150, 200, 0.8)"
      ctx.lineWidth = 3
      ...
      ctx.strokeStyle = isDark ? "rgba(100, 150, 255, 0.4)" : "rgba(80, 130, 235, 0.5)"
      ctx.lineWidth = 1
      ctx.beginPath()
  ...
  return (
    <div className="space-y-4" role="region" aria-label="Schrödinger's cat visualization">
      <canvas
        ...
        aria-label="Schrödinger's cat: quantum superposition visualization"
        role="img"
        aria-live="polite"
        aria-atomic="true"
      />
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        >
          <div className={isDark ? "text-red-400 font-semibold" : "text-red-700 font-semibold"}>
            💀 Dead
          </div>
          <div className={isDark ? "text-white font-mono" : "text-gray-900 font-mono"}>
